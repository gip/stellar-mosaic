...
// it does no IO. Ported verbatim from the frontend `orderPlan.ts` (only the Note import retargets to
...
export function planAssembly(notes: Note[], assetId: number, targetRaw: bigint): AssemblyPlan {
  ...
  if (total < targetRaw)
    return { kind: "impossible", reason: "Amount exceeds your spendable balance." };
  ...
  const covering = sp
    .filter((n) => BigInt(n.amount) > targetRaw)
    .sort((x, y) => {
