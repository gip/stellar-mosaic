// System prompt for a runtime agent, adapted from agents/src/prompts.ts. The protocol conventions
// are load-bearing (the mirror rule is what makes two orders cross for an atomic full fill); the
// agent's mandate comes from the web-configured prompt, this only teaches identity, protocol, and
// workflow.

import type { RuntimeContext } from "./context.js";

export function systemPrompt(ctx: RuntimeContext): string {
  const peers = ctx.config.peers ?? [];
  const peerList =
    peers.length > 0
      ? peers.map((p) => `  - "${p.name}" — XMTP (ethereum) address ${p.eth_address}`).join("\n")
      : "  (none configured — you trade alone; skip all peer coordination)";
  const symbolById = new Map(ctx.desk.assets.map((a) => [a.asset_id, a.symbol]));
  const assetList = ctx.desk.assets
    .map((a) => `asset ${a.asset_id} = ${a.symbol}${a.token === "native" ? " (native)" : ""}`)
    .join(", ");
  const pairNames = ctx.desk.pairs.map((p) => `${symbolById.get(p.base_asset)}/${symbolById.get(p.quote_asset)}`);
  const pairList = ctx.desk.pairs
    .map((p, i) => `pair ${i} = ${symbolById.get(p.base_asset)}/${symbolById.get(p.quote_asset)} (base/quote)`)
    .join(", ");
  const markets = pairNames.join(", ");
  const marketData = ctx.config.params?.webSearch
    ? `
MARKET DATA
...
`
  ...
  return `You are an autonomous OTC trading agent named "${ctx.name}", settling private ${markets}
trades on Stellar via Mosaic (a privacy DEX). Counterparties are reachable only over XMTP.

IDENTITY
- Your Stellar account: ${ctx.identity.stellarPublicKey}
- Your peers:
${peerList}
${marketData}
