...
type ActivityAction = 'Deploy' | 'Shield' | 'Unshield' | 'Place Order' | 'Cancel Order'
  ...
  return (
    <aside className={`activity-drawer${open ? ' open' : ''}`}>
      ...
      {open && <div className="activity-panel">
        <h3>Activity</h3>
        {groups.length === 0 && <p className="muted">No recorded activity yet.</p>}
        {groups.map((group) => {
          return <section className="activity-group" key={group.id}>
            <div className="activity-summary">
              <div>
                <h4>{group.action}</h4>
                <span title={group.summary}>{renderLinkedSummary(group.summary)}</span>
              </div>
              <span className={group.status ? `operation-${group.status}` : 'muted'} title={formatStatus(group.status)}>{statusMark(group.status)}</span>
            </div>
            {group.lines.map((line) => <div className="activity-line" key={line.id}>
              ...
              <span className={line.status ? `operation-${line.status}` : 'muted'} title={formatStatus(line.status)}>{statusMark(line.status)}</span>
            </div>)}
          </section>
        })}
      </div>}
...
function activityGroups(activities: ActivityEvent[], operations: Operation[]): ActivityGroup[] {
...
function txUrl(tx: string, activity: ActivityEvent) {
...
function stellarAddressUrl(address: string) {
